...
export default function Create({
  ...
  const { mutate: insertTech } = api.technologies.insert.useMutation();
    ...
    <Page title="Create">
      <main className="flex min-h-screen w-full flex-col items-center bg-slate-800">
        <Wrapper className="flex items-center">
          <Header user={user} />
          <div className="mt-8 flex w-full flex-col items-center rounded border p-4 pt-8">
            <h2 className="text-3xl font-medium text-white">Project</h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                setErrors({
          ...
          <div className="my-8 flex w-full flex-col items-center rounded border p-4 pt-8">
            <h2 className="text-3xl font-medium text-white">Technology</h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                if (techName === "") {
                ...
                techName !== "" &&
                  //create new tech with name specified
                  insertTech(
                    {
                      name: techName,
                    },
                    {
                      onSuccess: (e) => {
                        void router.push(
                          `/?type=success&message=Successfully created new technology`
                        );
                      },
                      onError: (e) => {
                        void router.push(
                          `/?type=error&message=Failed to create new techoology`
                        );
                      },
                    }
                  );
              ...
              <div className="-mx-3 flex flex-wrap">
                <div className="w-full px-3 md:mb-0">
                  ...
                  >
                    Name
                    {techError && (
                      <span className="text-red-600">{techError}</span>
                    )}
                  </label>
                  <input
                    className="mb-3 block w-full appearance-none rounded border border-gray-200 bg-gray-200 px-4 py-3 leading-tight text-gray-700 focus:border-gray-500 focus:bg-white focus:outline-none"
